...
<h1 class="text-xl font-bold mb-5">Get a Launchtube token</h1>

<form class="flex flex-col items-start" on:submit|preventDefault={handleSubmit}>
    <label class="mb-2">
        Agree to
        <a
            href="https://launchtube.xyz/terms-and-conditions"
    ...
    <div class="flex">
        <label class="flex flex-col">
            ...
            <div class="flex items-center border mr-2 font-mono text-sm">
                <input
                    class="pl-2 py-1 bg-white"
                    type="number"
                    min="10"
                    max="1000"
                    bind:value={amount}
                    on:change={clampAmount}
                />
        ...
        <label class="flex flex-col">
            You sell
            <button
                class="group flex items-center bg-green-700 text-white px-2 py-1 mr-2 disabled:bg-gray-400"
                type="submit"
                disabled={loading}
                >Sell{loading ? 'ing...' : ''} <span
                    class="bg-white text-green-700 font-mono text-xs ml-2 px-2 rounded-full group-disabled:text-gray-400"
                    >{(amount * KALE_TO_XLM_FACTOR).toLocaleString()} KALE</span
                ></button
            >
        </label>
    </div>
...
{#if lt_jwt}
    <pre
        class="whitespace-pre-wrap break-words text-sm max-w-[500px] mt-5"><code
            >{lt_jwt}</code
        ></pre>
    <button class="text-white bg-black px-2 py-1 mt-2" on:click={copyToken}
        >Copy</button
    >
{/if}
